...
if((isset($_POST['test-form-mail'])&&$_POST['test-form-mail']!="")&&(isset($_POST['test-form-name'])&&$_POST['test-form-name']!="")&&(isset($_POST['ball'])&&$_POST['ball']!="") ){
    //Проверка отправилось ли наше поля name и не пустые ли они
    $to = 'granevich@gmail.com'; //Почта получателя, через запятую можно указать сколько угодно адресов
    $subject = 'Обратный звонок. Тест'; //Загаловок сообщения
    $message = '
                <html>
                    <head>
                        <title>'.$subject.'</title>
                    ...
                    <body>
                        <p>Имя: '.$_POST['test-form-name'].'</p>
                        <p>Email: '.$_POST['test-form-mail'].'</p>
                        <p>Points: '.$_POST['ball'].'</p>
                    </body>
                </html>'; //Текст нащего сообщения можно использовать HTML теги
    $headers  = "Content-type: text/html; charset=utf-8 \r\n"; //Кодировка письма
    $headers .= "From: Отправитель <from@example.com>\r\n"; //Наименование и почта отправителя
